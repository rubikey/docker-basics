...
1.  **Gateway (Reverse Proxy):** **Nginx Proxy Manager** listening on ports `80`/`443`. It handles domain resolution (`my-app.local`, `logs.local`), SSL termination, and routes traffic to internal containers.
...
3.  **Data (Backend & Security):**
    ...
    * **Automated Backups:** A sidecar container (`fradelg/mysql-cron-backup`) that performs daily database dumps at **03:00 AM**. Backups are compressed and retained for 7 days.
